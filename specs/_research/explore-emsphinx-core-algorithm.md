...
**Addendum (2026-09-02, `specs/2026-09-02-spherical-refinement/requirements.md` D4, D5)**: `maxIter` is **15** (`:448`; the 25 elsewhere quoted for a refinement Newton loop belongs to `interpolateMaxima`'s own Newton, `:1312` -- a different loop, owned by Phase 4). `prevMag2 = 2*pi*3/slP` is a recorded C++ quirk: the comment promises "first step better not be more than 1 pixel in each direction" but the bound compares `|step|^2` against a *linear* quantity -- at `slP` 135 it admits a first step of ~8 grid cells (a one-cell-per-axis step is `mag2 = 3 (2*pi/slP)^2`); ported verbatim (section 8 item 41). The inner `if(det < euEps)` at `:478` is always true when reached ("singular matrix" vs "converging to saddle" differ in message only -- both throw). On failure the returned *value* is `derivatives(eu0, der=false)` -- the analytic value at the start, **not** the interpolated peak estimate -- and on success the value lags the final sub-`absEps` step (`peak` is assigned before the step is applied, `:457`, `:487`). A far start that *converges* may land on a stationary point with a **lower** value (the 1x1/2x2 fallbacks freeze `step[2]` and check only `det >= euEps`, so a converged fixed point need not be a maximum; measured at `bw` 24: 3 of 4 converged far starts decrease) -- Newton is local, score-monotone only for starts from the coarse pipeline.
...
**Addendum (2026-09-02, `specs/2026-09-02-spherical-refinement/requirements.md` D1)**: the `deg` flag (`:909`) is dead code -- no call site sets it. `mBW < bw` would misread the factor tables (`wigE`/`wigW`/`wigB` and `dBeta` are strided by the `bw` they were built for); every EMSphInx call site passes `bw`, and the port's kernel takes a single `bandwidth` with spectrum shapes validated before entry (with `BOUNDSCHECK` off a mismatch is silent garbage, section 8 item 42).
...
**Addendum (2026-09-02, `specs/2026-09-02-spherical-indexing-ebsd/requirements.md` D3)**: the peak *value* is invariant under a rotation of the reference (a rotated master matches the same pattern at a compensated rotation), so a rotated copy of a master is degenerate as a second phase -- measured at `bw` 68: the true phase wins 6/9 with score gaps -0.0151 to +0.0090. A *phase*-scrambled decoy (`flm * exp(i U(0, 2 pi))`) is not the spectrum of a real spherical function (the `m = 0` row must stay real; 0.99 relative round-trip error); the discrimination decoy is a **sign**-scrambled copy (`alm * default_rng(42).choice([-1.0, 1.0], ...)`: every `|a_lm|` preserved, `m = 0` real, 9/9 with gaps 0.2970-0.4151), the rotated copy staying as a degeneracy control (section 8 item 38).
...
**Addendum (2026-09-02, `specs/2026-09-02-spherical-refinement/requirements.md` D5)**: **`refineImage` discards its refinement** -- `:296` drops the returned `Result` and `eu` is `const`, so the refine-only work item (`idx.hpp:438-450`) stores the *unrefined* orientation, and the stored `corr` is **0 or stale, not uninitialised**: `std::vector<Result> res(om.size())` value-initialises (zero-initialising `corr` -- `Result` is an aggregate with no default member initialisers) and `res` is hoisted outside the per-pattern loop (`idx.hpp:406-407`), so a pure `msk & 0x02` run stores 0 and a mixed 0x01/0x02 batch stores the previous pattern's `indexImage` score (section 8 item 40). Phase 7's `EBSD.refine_orientation_spherical` implements the documented *intent* -- refine the stored orientation, store the refined score -- a recorded deviation; Phase 10's parity harness must not compare refine-only outputs against `IndexEBSD.exe` `msk & 0x02` runs.
...
**Addendum (2026-09-02, `specs/2026-09-02-spherical-indexing-ebsd/requirements.md` D2, D3)**: (i) `quNp` is identity as shipped (section 8 item 10), so the final conversion of `indexImage` (`zyz2qu -> quNp mul -> conjugate`, `:264-269`) collapses to `~Rotation(zyz2qu(zyz))` -- exactly `_euler.rotation_from_zyz` (Phase 6). (ii) The per-phase candidate count is **one** (each phase contributes only its global correlation peak), so `n_best > n_phases` rows keep the invalid fill ("extra points will be filled with an invalid phase", `:103`); pseudo-symmetry adds candidates in Phase 8. (iii) The zero-seeded fill (`res[i] = {corr: 0, phase: -1}`) plus the descending `upper_bound` insertion (`:217-239`) means a candidate with `corr <= 0` is **never recorded** ("only keep something with a positive phase", `:219`) -- the port drops such candidates explicitly; negative scores are reachable (the window-mask correlation of a constant float measured -2.64).
...
**Addendum (2026-09-02, `specs/2026-09-02-spherical-indexing-ebsd/requirements.md` D1, D2)**: (i) the geometry's `sample_tilt` comes *from the master* (`sampleTilt(phases[0].getSig())`, `idx.hpp:218`), so a harmonics-vs-detector tilt mismatch is structurally impossible in EMSphInx -- a port that takes the geometry from an `EBSDDetector` must re-establish the binding as a guard (measured: harmonics at 70 deg against a 65-deg detector index 4.68 deg wrong at *higher* scores, i.e. silently without the guard). (ii) The `ebsdWorkItem` catch (`:427-437`) writes `qu = identity, metric = 0, imQual = 0, phase = -1` on any per-pattern exception; Phase 6 ports these semantics and extends them to `ptp == 0` degeneracies with the measured garbage-score rationale -- a deviation for the constant-uint8/AHE-ripple case only (EMSphInx would correlate the rounding ripple at score +0.23, a garbage orientation; section 8 item 37), parity for the constant-float case (its -2.64 window-mask score is dropped by the insertion rule anyway, only earlier).
...
37. **AHE constant ripple defeats `unproject`'s mask branch** (`ahe.hpp`; Phase 6, `specs/2026-09-02-spherical-indexing-ebsd/requirements.md` D2): the AHE four-term interpolation of a uniform image returns `255 + O(1e-13)` (the Phase 5 finding behind item 35's "uniform image maps to 255"), never an exact constant, so a constant uint8 pattern never reaches `unproject`'s `ptp == 0` mask branch (item 33) through the default pipeline -- EMSphInx normalises the ripple and indexes it (measured iq ~1.0, score +0.23, garbage orientation); the Phase 6 indexer intercepts it with raw and processed `ptp` guards, the one genuine deviation (the constant-float/`n_regions=0` case is parity: its measured -2.64 window-mask score is dropped by the C++'s own zero-seeded insertion rule) (sections 6.2/6.3 addenda).
38. **Rotated-copy degeneracy** (`sht_xcorr.hpp`; Phase 6, `specs/2026-09-02-spherical-indexing-ebsd/requirements.md` D3): the correlation peak *value* is invariant under a rotation of the reference, so a rotated master copy is degenerate as a second phase (measured gaps +-0.015 at `bw` 68), and a *phase*-scrambled copy is not a real spherical function -- discrimination tests use a sign-scrambled copy and keep the rotated copy as a degeneracy control (section 3.10 addendum).
39. **`resize`-vs-direct normalisation difference** (`idx/master.hpp:601-614`; Phase 6, `specs/2026-09-02-spherical-indexing-ebsd/requirements.md` D1): `MasterSpectra::resize` truncation of a stored spectrum (`resize(120 -> 68)`) differs from building directly at the target bandwidth (`from_master_pattern(bw=68)`) by 2.9 % max-abs / 10.3 % rel-L2 / up to ~100 % per significant coefficient (the weighted normalisation depends on the analysis bandwidth) -- parity runs must resize from the stored `.sht` bandwidth exactly as `IndexEBSD` does (`idx.hpp:182`).
40. **`refineImage` discards its refinement** (`idx/indexer.hpp:296`; Phase 7, `specs/2026-09-02-spherical-refinement/requirements.md` D5): `refine(res.phase, gln.data(), eu)` drops the returned `Result` and `eu` is `const`, so the refine-only work item (`idx.hpp:438-450`) stores the *unrefined* orientation with a **zero or stale** score (`res` is value-initialised and hoisted outside the per-pattern loop, `idx.hpp:406-407`: a pure `msk & 0x02` run stores 0, a mixed 0x01/0x02 batch the previous pattern's `indexImage` score). Phase 7 implements the documented intent (refine, store the refined score) -- a recorded deviation; Phase 10 must not compare refine-only outputs against `IndexEBSD.exe` `msk & 0x02` runs (section 6.2 addendum).
41. **`refinePeak`'s `prevMag2` unit mix** (`sht_xcorr.hpp:450`; Phase 7, `specs/2026-09-02-spherical-refinement/requirements.md` D4): the first-step bound `prevMag2 = 2*pi*3/slP` compares `|step|^2` against a *linear* quantity (the comment promises "not ... more than 1 pixel in each direction"), admitting a first step of ~8 grid cells at `slP` 135 (a one-cell-per-axis step is `mag2 = 3 (2*pi/slP)^2`). Ported verbatim, documented (section 3.6 addendum).
42. **Shape mismatch is silent garbage with `BOUNDSCHECK` off** (a porting lesson; Phase 7, `specs/2026-09-02-spherical-refinement/requirements.md` D1): feeding `(68, 68)` spectra to a `bw` 88 `_derivatives` kernel returned values of `1e225`-`1e287` and NaN Hessians (out-of-bounds reads), not an error -- every public entry (`refine_zyz`) validates spectrum shapes before the kernel, and the factor tables/`d_beta` are built by the instance itself, never caller-supplied (section 3.7 addendum).
43. **The normalised refined score can dip below the coarse one** (`sht_xcorr.hpp:263-264`; Phase 7, `specs/2026-09-02-spherical-refinement/requirements.md` D6): the Newton step maximises the *un-normalised* correlation and the chain rule through the moving window is not applied (the C++'s own documented caveat, ported as-is), so the refined *normalised* score can end below the coarse score -- measured 4 of 165 large-map points, worst -4.8e-4; never on the small map (section 3.9).
44. **`PatternRepack.exe` writes no `Manufacturer`, and the reader wants vlen ASCII** (`pattern_repack.cpp`, `pattern.hpp:608-637`; Phase 9, `specs/2026-09-02-sht-interop/requirements.md` D1, D2): the repacker's own output is refused by `IndexEBSD.exe` until a root `Manufacturer` dataset is added (measured error text), so `write_emsphinx_patterns` always writes one -- a **scalar vlen-ASCII** dataset: a vlen **UTF-8** `Manufacturer` (the h5py default) is fatal with a misleading `H5Dread` error (`GetVendor`'s fixed-length fallback `:631-635` throws into the outer handler `:519` -- measured) (1.6/3.3 addenda of the programs-and-formats doc).
45. **Only uint8 HDF5 pattern files are read correctly** (`pattern.hpp:494`, `:515`; Phase 9, `specs/2026-09-02-sht-interop/requirements.md` D2): the mmap gate `0 != getNfilters()` is inverted and unsatisfiable for contiguous datasets, so every HDF5 pattern file takes the buffered branch, which reads `NATIVE_UINT8` for any dtype -- uint16/float32 patterns are corrupted (measured 38.9 deg median garbage end-to-end); **filters are the real constraint** (gzip-compressed `/patterns` is fatal, chunked-unfiltered and alloc-late are fine -- measured); byte-swapped dtypes are rejected (`NATIVE_*` comparison `:476-480`, measured); the writer warns on non-uint8 writes and Phase 10's regression files must be uint8 (3.3 addendum of the programs-and-formats doc).
46. **File `Manufacturer` and nml `vendor` are decoupled** (`pattern.hpp:463-471`, `idx.hpp:221-229`; Phase 9, `specs/2026-09-02-sht-interop/requirements.md` D1, D6): the file's `Manufacturer` drives only the read-flip (EDAX/EMsoft flip, Oxford/Bruker/Bruker Nano/DREAM.3D no flip), the namelist's `vendor` only the PC interpretation -- measured with an EDAX-manufacturer file under a Bruker-vendor namelist, which indexes correctly; a wrong flip pairing gives ~39.6 deg median garbage.
47. **The two correct flip routes are equivalent but not bitwise** (Phase 9, `specs/2026-09-02-sht-interop/requirements.md` D7): the EMsoft-manufacturer/unflipped-write route and the Bruker-manufacturer/pre-flipped-write route agree only to ~0.01-0.02 deg (measured on the acid test: 0.7245/0.9479 vs 0.713/0.947 deg median/max vs the stored xmap, scores mean 0.6283 vs 0.6304) -- the canonical route for byte-stable Phase 10 references is the writer default (`Manufacturer` EMsoft, rows unflipped, nml vendor Bruker with `pc_average`, `nthread=1 batchsize=1`).
48. **`IndexEBSD.exe` at `nthread=1 batchsize=1` is bitwise deterministic run-to-run given a fixed fftw.wisdom** (Phase 10, `specs/2026-09-03-spherical-indexing-emsphinx-regression/requirements.md` D4, D8): the programs plan with `FFTW_PATIENT` (`square_sht.hpp:384`, `sht_xcorr.hpp:366`) and import/export the machine-wide wisdom at start/exit (`util/fft.hpp:71-108, 281-330`) -- PATIENT picks algorithms by timing and different algorithms round differently, so the bit pattern is a function of the wisdom state (md5 recorded beside the reference md5s); the shared file is also a race -- serialize runs.
49. **The delta axis is inert on every vendor route** (Phase 10, `specs/2026-09-03-spherical-indexing-emsphinx-regression/requirements.md` D1): the EMsoft-vendor pair {250, 500} through `.6g` namelists is bitwise identical in output (`sDst` scales exactly in decimal: 15021.2/7510.6/3755.3) and the round-tripped pc is bit-identical for delta {125, 250, 500} -- delta invariance is a property of the kikuchipy conversion, pinned as a namelist round-trip unit test, not a reference scenario; the EMsoft-vs-Bruker route rounding differs at 1e-6 in pc = 5.8e-5 deg.
50. **EMSphInx's DCT IQ equals kikuchipy's ported IQ to float32 quantisation** (Phase 10, `specs/2026-09-03-spherical-indexing-emsphinx-regression/requirements.md` D6): <= 1.4e-8 over 165 points and every preprocessing variant, incl. the non-dividing `nregions` 7 remainder path at 6.6e-9 -- IQ is the sharpest cross-engine preprocessing probe; but it is chained to the exact uint8 pattern bytes: one gray level in one pixel moves that pattern's IQ by up to 5.2e-5 (AHE histogram cascade, 24-trial scan), so cross-engine CI bands must sit above the platform-drift budget (~2e-4 under numba fastmath) and below the smallest real signature (1.5e-2).
51. **Item 31 measured end-to-end** (Phase 10, `specs/2026-09-03-spherical-indexing-emsphinx-regression/requirements.md` D7): emulating the `bilinearCoeff` stretch in pc space (`pc' = (0.5/w + pc_x (w-1)/w, 0.5/h + pc_y (h-1)/h, pc_z (w-1)/w)`) collapses the kikuchipy-vs-IndexEBSD refined median from 0.3404 to 0.0940 deg (coarse 0.5096 -> 0.0745, large165 0.3390 -> 0.0717; scale-scan minimum exactly at `(w-1)/w`), i.e. ~75-85 % of the cross-engine residual is this documented sampling-convention difference and the true interpolation/Newton floor is ~0.07-0.09 deg; the emulation worsens stored-xmap agreement (0.365 -> 0.584 deg), so the port keeps kikuchipy's physical convention.
52. **The psym machinery is inert in every shipped CLI** (`idx/indexer.hpp:74, 175-180, 228, 243`; Phase 8, from the 2026-09-06 reports, `specs/2026-09-06-pseudo-symmetry/requirements.md` D1): at `master` 60f3517 and upstream release 87b2387 `Indexer::pSym` is only declared, reserved, placeholder-pushed and read -- nothing wires `MasterData::pSm` into it, and `clone()` (`:120`) rebuilds without it -- so setting `psymfile` loads operators and allocates extra output scans (`idx.hpp:249-250`) whose loop iterates empty lists; the fix is the user's own two-line `feature/GPU` wiring (`idx.hpp:386` -- `idx->pSym[i] = phases[i].pseudoSym()` -- and `indexer.hpp:244` -- `clone()` copies `pSym`; scoring logic there byte-identical to master), which is the Phase 8 reference baseline; the inertness itself is pinned by a local-gated binary test (D8.3).
53. **The D2 equivalence derivation** (`indexer.hpp:242-249`, `master_xcorr.cpp:145`; Phase 8, `specs/2026-09-06-pseudo-symmetry/requirements.md` D2): EMSphInx's variant composition `qp = q0 * q_file` applied to MasterXcorr-printed quaternions equals the NCC composition `op * rot` with `op = rotation_from_zyz(zyz_peak) = ~q_printed` -- the literal upstream chain is self-consistent despite the C++ author's own conjugation doubt (`indexer.hpp:244-247`), and the port keeps it with the conjugation placed explicitly at the psymfile boundary (write rows `(~op).data`, read `~Rotation(rows)`); the write direction is analytically derived and empirically unverifiable against the inert shipped binary (item 52).
54. **MasterXcorr defects** (`master_xcorr.cpp`; Phase 8, `specs/2026-09-06-pseudo-symmetry/requirements.md` D3): the candidate scan flat-indexes the cube with `sl = 2*bw - 1` while the correlator's true side is `slP = fastSize(2*bw - 1)` (`:86, 108-111, 201` vs `sht_xcorr.hpp:376-382`) -- silently mis-indexed whenever they differ (equal for every recommended bandwidth; NOT reproduced: the port scans the true `(bwP, slP, slP)` cube); the SVG crystallographic test is the self-comparison `fabs(nFld - nFld) < rotCut` (`:267`), always true, so the `addDot` branch is dead code; the usage string (`:44`) is stale (the real usage is `MasterXcorr <bw> <cutoff> <master1> [master2]`); the operator list is stdout-only and unfiltered (the `pseudo.svg` symmetry/dedup filters `:232-261` never apply to the printed rows, which include true-symmetry operators -- hence `exclude_symmetry=False` is the parity setting and `True` is a recorded kikuchipy deviation).
55. **`nml.hpp:247` never `ipath`-prefixes `pSymFile`** (Phase 8, `specs/2026-09-06-pseudo-symmetry/requirements.md` D1): `pSymFile` resolves relative to the CWD, while the same line prepends `ipath` to `patFile` a SECOND time when `psymfile` is set -- the psym face of the double-`ipath` quirk already ported in Phase 9's namelist (raw path storage, derived `pat_path`); binary psymfile runs must leave `ipath` empty or use CWD-relative paths.
