...
- ~~`rotate(Rotation.identity())` -> `NotImplementedError` matching "Phase 3"~~ (superseded 2026-09-07: Phase 8 implements `MasterPatternHarmonics.rotate`, so the pinned `NotImplementedError` test is replaced by the D7 contract tests of `specs/2026-09-06-pseudo-symmetry/requirements.md` -- identity no-op, composition, synthesis-direction oracle, flag neutralization, round trip, `rotate_harmonics` equivalence; plan 0.5 there updates this recorded pin where it lives); `__repr__` equals `"MasterPatternHarmonics: bw = 384, ni (m-3m), 20.1 keV, 70.0 deg"` for the in-package master.
...
- CI `windows-latest-py3.14` xdist workers crashed intermittently (15 access violations in one run, none in the next) inside orix's `qu_conj_gufunc` -- `@nb.guvectorize("(n)->(n)", cache=True)` without a signature, i.e. a dynamic gufunc compiled on first call and written to the shared numba cache; four workers racing on that cache on a fresh runner load a half-written kernel. Upstream's own tests (`test_ebsd_detector.py`, `test_ebsd_refinement.py`) crashed the same way. Mitigation in the root `conftest.py`: each xdist worker gets its own `NUMBA_CACHE_DIR` (`<tmp>/kikuchipy-numba-cache/<worker>`), set before numba is imported; no effect without xdist. Also closes the "sporadic wide-spectrum failures" the conventions reviewer saw locally.
