...
- Bandwidth vs source resolution: `get_spherical_harmonics`/`from_master_pattern` warn when `bandwidth > (source_dim - 1)/2` (a 401-px Lambert master supports bw <= 200; bw 384 fixtures exist only for mp2sht parity, never for accuracy claims). Default bandwidth 384 (EMSphInx mp2sht default).
